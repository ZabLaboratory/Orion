...
func TestEffects_HTTPRequestResumesContinuation(t *testing.T) {
	...
	prog := &ExecProgram{
		...
		Nodes: map[string]*ExecNode{
			...
			"set.out": setFromPin("set.out", "out", "req", "response",
...
func TestEffects_TwoInFlightResumeTheirOwnContinuations(t *testing.T) {
	...
	prog := &ExecProgram{
		...
		Nodes: map[string]*ExecNode{
			...
			"set.slow": setFromPin("set.slow", "out", "req.slow", "response", nil),
			"set.fast": setFromPin("set.fast", "out2", "req.fast", "response", nil),
		},
	...
	prog := &ExecProgram{
		...
		Nodes: map[string]*ExecNode{
			"rd": {ID: "rd", Op: OpSourceRead,
				Config: map[string]json.RawMessage{"source": raw(`"prices"`)},
				Next: map[string]ExecTarget{
					"then":  {Node: "set.out"},
					"error": {Node: "set.err"},
				}},
			"rd.bad": {ID: "rd.bad", Op: OpSourceRead,
				Config: map[string]json.RawMessage{"source": raw(`"undeclared"`)},
				Next:   map[string]ExecTarget{"error": {Node: "set.err"}}},
